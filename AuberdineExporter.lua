    ...
    if exportableCharacters then
        for charKey, charData in pairs(exportableCharacters) do
            ...
            local characterExport = {
                ...
                -- Équipement porté du personnage (schéma auberdine.eu existant)
...
-- Équipement porté, talents, quêtes (v1.5.0)
    ...
    elseif event == "PLAYER_EQUIPMENT_CHANGED" then
        if IsValidRealm() then
            ...
            if charKey and AuberdineExporterDB.characters[charKey] then
                -- Equipment porté (champ .equipment, schéma auberdine.eu existant)
                AuberdineExporterDB.characters[charKey].equipment = GetCharacterEquipment()
                AuberdineExporterDB.characters[charKey].lastUpdate = time()
            end
